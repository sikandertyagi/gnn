...
────────────────
  0       → benign
  1 or 2  → malicious  (treated as a single positive class)

All evaluation is binary: label > 0 = malicious, label == 0 = benign.
...
────────────────
  ...
  Per-class score statistics
    · mean / std / median / p95 for benign and malicious
...
def evaluate(
    ...
    metrics.update(n_total=n_total, n_benign=n_benign, n_malicious=n_pos)
    _row("Total events",  n_total)
    _row("Benign   (0)",  f"{n_benign}  ({100*n_benign/n_total:.1f}%)")
    _row("Malicious (1/2)", f"{n_pos}  ({100*n_pos/n_total:.1f}%)")

    # ── score distribution per class ──────────────────────────────────────────
    _section("Score Distribution by Class")
    dist_rows = []
    for mask, name in [(labels == 0, "Benign"), (labels > 0, "Malicious")]:
        if not mask.any():
            continue
        s = scores[mask]
        row = dict(name=name, count=int(mask.sum()),
    ...
    # ── detection rate at F1-optimal threshold ────────────────────────────────
    thr = best_f1_row["threshold"]
    _section("Detection Rate at F1-Optimal Threshold")
    mal_mask = labels > 0
    if mal_mask.any():
        det  = int((scores[mal_mask] >= thr).sum())
        tot  = int(mal_mask.sum())
        rate = det / tot
        metrics["detection_rate_malicious"] = rate
        _row("Malicious detected",   f"{det}/{tot}  ({100*rate:.1f}%)")
        _row("Malicious mean score", f"{scores[mal_mask].mean():.4f}")
    if n_benign > 0:
